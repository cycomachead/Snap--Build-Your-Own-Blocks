...
XML_Serializer.prototype.store = function (object, mediaID) {
    ...
    this.add(object);
    objData = object.toJSON(this, mediaID)
    objData._id = object[this.idProperty]
    return objData;
};
...
XML_Serializer.prototype.mediaXML = function () {
    ...
    this.media.forEach(function (object) {
        var str = object.toJSON(myself).replace(
            '~',
...
// Array.prototype.toJSON = function (serializer) {
...
StageMorph.prototype.toJSON = function (serializer) {
    ...
    return {
        tagName: 'project',
        _name: (ide && ide.projectName) ? ide.projectName : localize('Untitled'),
        _app: serializer.app,
        _version: serializer.version,
        notes: (ide && ide.projectNotes) ? ide.projectNotes : '',
        thumbnail: thumbdata,
        stage: {
            _name: this.name,
            _width: StageMorph.prototype.dimensions.x,
            _height: StageMorph.prototype.dimensions.y,
            _costume: this.getCostumeIdx(),
            _tempo: this.getTempo(),
            _threadsafe: this.isThreadSafe,
            _lines: SpriteMorph.prototype.useFlatLineEnds ? 'flat' : 'round',
            _codify: this.enableCodeMapping,
            _inheritence: this.enableInheritance,
            _scheduled: StageMorph.prototype.frameRate !== 0,
            pentrails: this.trailsCanvas.toDataURL('image/png'),
            costumes: serializer.store(this.costumes, this.name + '_cst'),
            sounds: serializer.store(this.sounds, this.name + '_snd'),
            variables: serializer.store(this.variables),
            blocks: serializer.store(this.customBlocks),
            scripts: serializer.store(this.scripts),
            sprites: serializer.store(this.children)
        },
        hidden: Object.keys(StageMorph.prototype.hiddenPrimitives),
        headers: code('codeHeaders'),
        code: code('codeMappings'),
        blocks: serializer.store(this.globalBlocks),
        variables: (ide && ide.globalVariables) ?
                serializer.store(ide.globalVariables) : ''
    };
};

SpriteMorph.prototype.toJSON = function (serializer) {
    ...
    data = {
        ...
        _scale: this.scale,
        _rotation: this.rotationStyle,
        _draggable: this.isDraggable,
        // this is different from existing serializers.
        hidden: this.isVisible,
        _costume: this.getCostumeIdx(),
        // or maybe: [this.color.r, this.color.g, this.color.b]
        _color: this.color,
        _pen: this.penPoint,
        costumes: serializer.store(this.costumes, this.name + '_cst'),
        sounds: serializer.store(this.sounds, this.name + '_snd'),
        variables: serializer.store(this.variables),
        blocks: !this.customBlocks ? '' : serializer.store(this.customBlocks),
        scripts: serializer.store(this.scripts),
    };
    ...
    if (this.exemplar) {
        data.inherit = {
            tagName: 'inherit'
            _exemplar: this.exemplar.name
        }
    }
    if (this.anchor) {
        data.nest = {
            tagName: 'nest',
            _anchor: this.anchor.name,
            _synch: this.rotatesWithAnchor,
            _scale: this.nestingScale
        }
    }
...
Costume.prototype.toJSON = function (serializer) {
    return {
        tagName: 'costume',
        _name: this.name,
        '_center-x': this.rotationCenter.x,
        '_center-y': this.rotationCenter.y,
        image: (this instanceof SVG_Costume ?
...
Sound.prototype.toJSON = function (serializer) {
    return {
        tagName: 'sound',
        name: this.name,
        sound: this.toDataURL()
    };
};

VariableFrame.prototype.toJSON = function (serializer) {
    var myself = this,
        data = [];


    return Object.keys(this.vars).forEach(function (vars, v) {
...
WatcherMorph.prototype.toJSON = function (serializer) {
...
ScriptsMorph.prototype.toJSON = function (serializer) {
    return this.children.reduce(function (xml, child) {
        ...
        if (child instanceof CommentMorph && !child.block) { // unattached
            return xml + child.toJSON(serializer);
        }
        return xml;
    }, '');
};

BlockMorph.prototype.toJSON = BlockMorph.prototype.toScriptXML = function (
    serializer,
    savePosition
) {
    var position,
...
BlockMorph.prototype.toBlockXML = function (serializer) {
    return serializer.format(
        ...
        this.comment ? this.comment.toJSON(serializer) : ''
    );
};

ReporterBlockMorph.prototype.toJSON = function (serializer) {
    return this.selector === 'reportGetVar' ? serializer.format(
        '<block var="@"/>',
        this.blockSpec
    ) : this.toBlockJSON(serializer);
};

ReporterBlockMorph.prototype.toScriptJSON = function (
    serializer,
    savePosition
) {
    var data, position,
    ...
    data = {
        tagName: 'script',
        contents: this.toJSON(serializer)
    };

    if (savePosition) {
            data._x = position.x / scale,
            data._y = position.y / scale,
    }
    return data;
};

CustomCommandBlockMorph.prototype.toBlockJSON = function (serializer) {
...
CustomBlockDefinition.prototype.toJSON = function (serializer) {
    var myself = this;
    ...
    function encodeScripts(array) {
        return array.reduce(function (xml, element) {
            ...
            if (element instanceof CommentMorph && !element.block) {
                return xml + element.toJSON(serializer);
            }
            return xml;
        }, '');
    }
...
ArgMorph.prototype.toJSON = function () {
    return {
        tagName: 'l',w
        contents: ''
    }; // empty by default
...
InputSlotMorph.prototype.toJSON = function (serializer) {
    if (this.constant) {
        return {
            tagName: 'l',
            contents: {
    ...
    return {
        tagName: 'l',
        contents: this.contents().text
    };
};

TemplateSlotMorph.prototype.toJSON = function (serializer) {
    return {
        tagName: 'l',
...
CommandSlotMorph.prototype.toJSON = function (serializer) {
...
FunctionSlotMorph.prototype.toJSON = CommandSlotMorph.prototype.toJSON;

MultiArgMorph.prototype.toJSON = function (serializer) {
    return {
        tagName: 'list',
        contents: serializer.store(this.inputs())
    };
};

ArgLabelMorph.prototype.toJSON = function (serializer) {
    return serializer.store(this.inputs()[0]); // was a % formatted
};

ColorSlotMorph.prototype.toJSON = function (serializer) {
    return {
        tagName: 'color',
        _r: this.color.r,,
        _g: this.color.g,,
...
List.prototype.toJSON = function (serializer, mediaContext) {
...
Context.prototype.toJSON = function (serializer) {
...
CommentMorph.prototype.toJSON = function (serializer) {
    var position,
        scale = SyntaxElementMorph.prototype.scale,
        data;
    ...
    data = {
        tagName: 'comment',
        _w: this.textWidth() / scale,
        _collapsed: this.isCollapsed,
        // name text different.
        text: serializer.escape(this.text()
    }

    if (!this.block) { // not attached to a block
